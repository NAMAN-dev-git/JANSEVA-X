...
const applicationInclude = {
  service: { select: { id: true, name: true, slug: true, isPrototype: true, requirements: { select: { id: true, name: true } } } },
  ...
  mockIssuedDocumentAttachments: { orderBy: { attachedAt: "asc" as const }, select: { id: true, requirementId: true, attachedAt: true, mockIssuedDocument: { select: { id: true, documentType: true, displayName: true, issuer: true, issueDate: true, expiryDate: true, status: true, isDemo: true } } } },
  generatedDocuments: { orderBy: { generatedAt: "asc" as const }, select: { id: true, documentType: true, originalFilename: true, signatureStatus: true, generatedAt: true, signedAt: true, signingSessions: { orderBy: { createdAt: "desc" as const }, take: 1, select: { status: true, expiresAt: true, completedAt: true } } } },
...
const assignableReviewStatuses: OfficerReviewStatus[] = [OfficerReviewStatus.NOT_ASSIGNED, OfficerReviewStatus.ASSIGNED];
...
const reviewApplicationInclude = {
  ...
  mockIssuedDocumentAttachments: { orderBy: { attachedAt: "asc" }, select: { id: true, requirementId: true, attachedAt: true, mockIssuedDocument: { select: { id: true, documentType: true, displayName: true, issuer: true, issueDate: true, expiryDate: true, status: true, isDemo: true } } } },
  generatedDocuments: { orderBy: { generatedAt: "asc" }, select: { id: true, documentType: true, originalFilename: true, signatureStatus: true, generatedAt: true, signedAt: true, signingSessions: { orderBy: { createdAt: "desc" }, take: 1, select: { status: true, expiresAt: true, completedAt: true } } } },
...
export class EmployeeRepository {
  ...
  async completed(input: ScopedQuery & CompletedQuery) {
    ...
    const [applications, total] = await prisma.$transaction([
      prisma.application.findMany({ where, skip: (input.page - 1) * input.limit, take: input.limit, orderBy: [{ [input.sortBy]: input.sortOrder }, { id: "asc" }], include: { ...registryInclude, statusHistory: { where: { status: ApplicationStatus.COMPLETED }, orderBy: { createdAt: "desc" }, take: 1, select: { createdAt: true } }, generatedDocuments: { orderBy: { generatedAt: "asc" }, select: { id: true, documentType: true, originalFilename: true, signatureStatus: true, generatedAt: true, signedAt: true, signingSessions: { orderBy: { createdAt: "desc" }, take: 1, select: { status: true, expiresAt: true, completedAt: true } } } } } }),
      prisma.application.count({ where }),
    ]);
    return { applications, total };
  }
  ...
  async assign(applicationId: string, officerId: string | null, userId: string) { return prisma.$transaction(async (tx) => { if (officerId) { const officer = await tx.officer.findFirst({ where: { id: officerId, isActive: true, user: { isActive: true } } }); if (!officer) return "OFFICER_NOT_FOUND" as const; } const app = await tx.application.findUnique({ where: { id: applicationId } }); if (!app) return null; if (app.status !== ApplicationStatus.SUBMITTED || !assignableReviewStatuses.includes(app.reviewStatus)) return "NOT_REVIEWABLE" as const; const updated = await tx.application.updateMany({ where: { id: applicationId, status: ApplicationStatus.SUBMITTED, reviewStatus: { in: assignableReviewStatuses } }, data: { assignedOfficerId: officerId, reviewStatus: officerId ? OfficerReviewStatus.ASSIGNED : OfficerReviewStatus.NOT_ASSIGNED, assignedAt: officerId ? new Date() : null } }); if (updated.count !== 1) return "NOT_REVIEWABLE" as const; await tx.applicationStatusHistory.create({ data: { applicationId, status: app.status, note: officerId ? "Application assigned for review" : "Application assignment released", changedByUserId: userId } }); return tx.application.findUnique({ where: { id: applicationId }, include: applicationInclude }); }); }
