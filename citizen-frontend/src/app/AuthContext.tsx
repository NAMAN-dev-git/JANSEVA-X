...
import { purgeExamOfflineRecordsForUser } from "../offline/exam-offline-db";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AuthState>(() => ({ user, loading, setSession: (nextUser, tokens) => { setTokens(tokens); setUser(nextUser); }, signOut: async () => { const refreshToken = getTokens()?.refreshToken; const signingOutUserId = user?.userId; try { if (refreshToken) await citizenApi.logout(refreshToken); } finally { if (signingOutUserId) await purgeExamOfflineRecordsForUser(signingOutUserId).catch(() => undefined); clearTokens(); setUser(null); } }, refreshUser }), [user, loading]);
